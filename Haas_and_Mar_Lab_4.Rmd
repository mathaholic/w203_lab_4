...
a1
a2 = lm(data$combinded_apgarscores ~ data$monpre)
...
Our AIC scores tell us that a1 is efficient than the other three, so let us exlore a1 further:

```{r}
plot(a1)
```

But there is quite a bit of clustering in this dataset's values.  Let's try a log transform:

```{r}
a4 = lm(log(data$combinded_apgarscores) ~ data$monpre + data$npvis)
summary (a4)
AIC(a4)
plot(a4)
...
Again, we did't get very good results.  We cannot look at the logs of monpre or npvis as they contain some zero values, and we suspect those datapoints are important as they appear to represent mothers who had no prenatal care.  Instead, let us try the product of the APGAR scores.
...
summary(a7)
summary(a8)
a9 = lm(data$normalized_combined_apgar~ data$npvis)
a10 =lm(data$normalized_product_apgar~ data$npvis)
summary(a9)
summary(a10)
```
...
```{r}
data$weight_apgar_ratio = data$bwght/data$combinded_apgarscores
a11 = lm(data$weight_apgar_ratio ~ data$npvis )
a12 = lm(data$weight_apgar_ratio ~ data$npvis + data$monpre)
summary(a11)
summary(a12)
AIC(a11)
AIC(a12)
...
```{r}
###these exclude NaN and Infs are not working :(
#data$weight_apgar_prod_ratio = data$bwght/data$product_apgarscores
#a13 = lm(data$weight_apgar_prod_ratio ~ data$npvis,na.action = NULL, singular.ok = #TRUE)
#a14 = lm(data$weight_apgar_prod_ratio ~ data$npvis + data$monpre, na.exclude(data), #singular.ok = TRUE)
#summary(a13)
#summary(a14)
#AIC(a13)
#AIC(a14)

#p <- ggplot(a14, aes(a14$residuals, a14$fitted)) + geom_point(size = 0.25) + 
...
```{r}
c1 = lm(data$lbwght ~ data$monpre + data$npvis + data$male +
          data$mwhte + data$fblck)
summary(c1)
plot(c1)
```
